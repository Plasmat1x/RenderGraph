...
public unsafe class DX12UploadHeapManager: IDisposable
{
  ...
  private readonly List<UploadBuffer> p_uploadBuffers = new();
  private readonly List<UploadBuffer> p_availableBuffers = new();
  ...
  /// </summary>
  public UploadBuffer AcquireBuffer(ulong _requiredSize)
  {
    lock(p_lock)
    {
  ...
  /// Вернуть буфер в пул
  /// </summary>
  public void ReleaseBuffer(UploadBuffer _buffer)
  {
    if(_buffer == null)
      return;
  ...
  private UploadBuffer CreateUploadBuffer(ulong _size)
  {
    ...
    var buffer = new UploadBuffer(resource, _size);
