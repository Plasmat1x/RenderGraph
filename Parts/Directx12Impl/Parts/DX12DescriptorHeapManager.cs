...
public unsafe class DX12DescriptorHeapManager: IDisposable
{
  ...
  private readonly DX12StaticDescriptorHeap p_rtvHeap;
  private readonly DX12StaticDescriptorHeap p_dsvHeap;
  private readonly DX12StaticDescriptorHeap p_srvHeap;
  private readonly DX12StaticDescriptorHeap p_samplerHeap;

  private readonly DX12DynamicDescriptorHeap p_gpuSrvHeap;
  private readonly DX12DynamicDescriptorHeap p_gpuSamplerHeap;
  ...
  public DX12DescriptorHeapManager(ComPtr<ID3D12Device> _device)
  {
    ...
    p_rtvHeap = new DX12StaticDescriptorHeap(
      _device,
      DescriptorHeapType.Rtv,
      256,
      false);

    p_dsvHeap = new DX12StaticDescriptorHeap(
      _device,
      DescriptorHeapType.Dsv,
      128,
      false);

    p_srvHeap = new DX12StaticDescriptorHeap(
      _device,
      DescriptorHeapType.CbvSrvUav,
      4096,
      false);

    p_samplerHeap = new DX12StaticDescriptorHeap(
      _device,
      DescriptorHeapType.Sampler,
      256,
      false);

    p_gpuSrvHeap = new DX12DynamicDescriptorHeap(
      _device,
      DescriptorHeapType.CbvSrvUav,
      1024);

    p_gpuSamplerHeap = new DX12DynamicDescriptorHeap(
      _device,
      DescriptorHeapType.Sampler,
      64);
  }

  public DX12DescriptorAllocation AllocateRTV(uint _count = 1)
  {
    return p_rtvHeap.Allocate(_count);
  }

  public DX12DescriptorAllocation AllocateDSV(uint _count = 1)
  {
    return p_dsvHeap.Allocate(_count);
  }

  public DX12DescriptorAllocation AllocateCBVSRVUAV(uint _count = 1)
  {
    return p_srvHeap.Allocate(_count);
  }

  public DX12DescriptorAllocation AllocateSampler(uint _count = 1)
  {
